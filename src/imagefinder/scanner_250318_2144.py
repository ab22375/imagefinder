...
from imagefinder.imageprocessor import load_image, ImageLoaderRegistry
...
def convert_raw_to_jpg_and_load(path: str) -> np.ndarray:
    ...
    try:
        ...
        methods = [
            ...
            convert_with_rawpy,  # Replaced rawtherapee with rawpy
        ]
        last_error = None
...
def convert_with_rawpy(path: str, output_path: str) -> bool:
    """Convert RAW to JPG using rawpy"""
    try:
        import rawpy
        import imageio
        ...
        logging.warning(f"rawpy conversion failed: {e}")
        return False
    
def convert_raw_to_jpg_and_load_250318_2040(path: str) -> np.ndarray:
    """Convert a RAW file to JPG and load it for hashing"""
    temp_dir = os.path.join(os.path.expanduser("~"), "temp")
    os.makedirs(temp_dir, exist_ok=True)
